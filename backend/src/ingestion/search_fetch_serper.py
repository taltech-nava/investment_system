...
def build_query(subject: str, intent: str, bank_block: list[str], title_block: list[str]) -> str:
    # Subject and Intent
    query = f'("{subject}")'
    if intent:
        query += f' ("{intent}")'
...
# --- 3. SERPER MANAGER ---
def fetch_links(query: str, num_results: int) -> list[dict]:
...
def download_corpus(subject: str, intent: str, candidates: list[dict]) -> None:
    ...
    for i, item in enumerate(candidates):
        ...
        try:
            ...
            if "pdf" not in content_type.lower():
                print(
                    f"      ({i + 1}/{len(candidates)}) SKIPPED — not a PDF by Content-Type: {item['url']}"
                )
                manifest.append(
                    {
                        "report_id": report_id,
            ...
            if not resp.content.startswith(b"%PDF-"):
                print(
                    f"      ({i + 1}/{len(candidates)}) SKIPPED — not a PDF by magic bytes: {item['url']}"
                )
                manifest.append(
                    {
                        "report_id": report_id,
...
if __name__ == "__main__":
    ...
    for sub in subjects:
        print(f"\n>>> Starting Job: {sub}")
        query = build_query(
            sub, SEARCH_PARAMS["intent"], SEARCH_PARAMS["bank_block"], SEARCH_PARAMS["title_block"]
        )
